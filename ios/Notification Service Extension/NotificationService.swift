...
class NotificationService: UNNotificationServiceExtension {
    ...
    override func didReceive(_ request: UNNotificationRequest, withContentHandler contentHandler: @escaping (UNNotificationContent) -> Void) {
        ...
        if let bestAttemptContent = bestAttemptContent {
            ...
            // os_log("[GaussInteractPushHelper] New message received: %{public}@", log: .default, type: .error, bestAttemptContent.userInfo)
            os_log("[GaussInteractPushHelper] New message received")
            ...
            guard let roomId = bestAttemptContent.userInfo["room_id"] as? String,
                ...
                os_log("[GaussInteractPushHelper] Room ID or Event ID is missing!")
