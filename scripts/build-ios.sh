...
[ -n "${GAUSSINTERACT_NEW_GROUP}" ] && {
	# App group IDs
	sed -i "" "s/group.${GAUSSINTERACT_ORIG_GROUP}.app/group.${GAUSSINTERACT_NEW_GROUP}.app/g" "ios/FluffyChat Share/FluffyChat Share.entitlements"
	sed -i "" "s/group.${GAUSSINTERACT_ORIG_GROUP}.app/group.${GAUSSINTERACT_NEW_GROUP}.app/g" "ios/Runner/Runner.entitlements"
	sed -i "" "s/group.${GAUSSINTERACT_ORIG_GROUP}.app/group.${GAUSSINTERACT_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
	# Bundle identifiers
	sed -i "" "s/${GAUSSINTERACT_ORIG_GROUP}.app/${GAUSSINTERACT_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
}

[ -n "${GAUSSINTERACT_NEW_TEAM}" ] && {
	# Code signing team
	sed -i "" "s/${GAUSSINTERACT_ORIG_TEAM}/${GAUSSINTERACT_NEW_TEAM}/g" "ios/Runner.xcodeproj/project.pbxproj"
}
cat << EOHELP
...
2. Fix it yourself
    ...
    - Ensure that Xcode created the App ID successfully (for gaussinteract and gaussinteract.share)
...
[ -n "${GAUSSINTERACT_INSTALL_IPA}" ] && {
  ...
  cfgutil --foreach install-app "${TMPDIR}/gaussinteract.ipa"
  rm -rf "${TMPDIR}"
}
